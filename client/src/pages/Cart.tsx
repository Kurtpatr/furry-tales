...
export default function Cart() {
  ...
  const cart = useCart();
  const createOrderMutation = trpc.account.createOrder.useMutation({
    onSuccess: () => {
      setCheckoutOpen(false);
      setOrderComplete(true);
      cart.clear();
    },
    onError: (error) => setNotice(error.message || "We could not place your order. Please try again."),
  });
  ...
  function placeOrder(event: React.FormEvent) {
    ...
    createOrderMutation.mutate({
      ...
      items: cart.items.map((item) => ({ productId: item.id, name: item.name, type: item.type, priceCents: Math.round(item.price * 100), quantity: item.quantity, image: item.image })),
    });
  }
