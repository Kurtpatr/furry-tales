...
function veterinaryLabel(serviceType: string) {
  return serviceType === "anti-rabies" ? "Anti-Rabies Vaccination" : serviceType === "full-checkup" ? "Full Checkup" : serviceType === "deworm" ? "Deworm" : "Checkup";
}
...
export default function Dashboard() {
  ...
  const ordersQuery = trpc.account.orders.useQuery(undefined, { enabled: isAuthenticated });
  const veterinaryQuery = trpc.account.veterinaryAppointments.useQuery(undefined, { enabled: isAuthenticated });
  ...
  const tabLabel = { overview: "Overview", pets: "My pets", services: "Services", orders: "Order history", profile: "Profile" }[activeTab];
    ...
    <div className="dashboard-shell">
      <aside className={`dashboard-sidebar ${sidebarOpen ? "dashboard-sidebar--open" : ""}`}>
        ...
        <nav className="dashboard-nav" aria-label="Dashboard navigation">
          ...
          <button className={activeTab === "orders" ? "is-active" : ""} onClick={() => selectTab("orders")}><ShoppingBag size={19} /> Order history <b>{orders.length}</b></button>
      ...
      <main className="dashboard-main">
        ...
        <div className="dashboard-content">
          ...
          {activeTab === "services" && <section className="dashboard-tab-content"><div className="service-tab-grid"><div className="dashboard-panel service-menu-panel"><div className="panel-heading"><div><span className="eyebrow">Book a visit</span><h2>Choose care</h2></div></div><button className="service-option" onClick={() => setNotice("Open Veterinary from the home Services section to choose care") }><span className="service-option-icon service-option-icon--lavender"><Stethoscope size={22} /></span><span><strong>Veterinary</strong><small>Checkups, vaccines, deworming</small></span><CaretRight size={18} /></button><button className="service-option" onClick={() => setNotice("Grooming booking is ready for the next step")}><span className="service-option-icon service-option-icon--peach"><Scissors size={22} /></span><span><strong>Grooming</strong><small>Baths, trims, nails, packages</small></span><CaretRight size={18} /></button><button className="service-option" onClick={() => setNotice("Daycare booking is ready for the next step")}><span className="service-option-icon service-option-icon--mint"><House size={22} /></span><span><strong>Daycare</strong><small>Half-day, full-day, drop-off</small></span><CaretRight size={18} /></button></div><div className="dashboard-panel service-upcoming-panel"><div className="panel-heading"><div><span className="eyebrow">Your care plan</span><h2>Upcoming</h2></div><Clock size={25} className="panel-heading-icon" /></div><div className="service-timeline"><div className="timeline-line" /><div className="timeline-item"><span className="timeline-dot timeline-dot--violet" /><div><strong>Grooming</strong><small>Friday, September 26 · 10:30 AM</small><span className="status-pill status-pill--pending">Pending confirmation</span></div></div><div className="timeline-item"><span className="timeline-dot timeline-dot--mint" /><div><strong>Veterinary checkup</strong><small>Due in 14 days</small><span className="status-pill status-pill--ready">Reminder set</span></div></div></div><div className="service-note"><Bell size={18} /><span>We’ll notify you before each visit.</span></div></div></div><div className="dashboard-panel veterinary-history-panel"><div className="panel-heading"><div><span className="eyebrow">Veterinary care</span><h2>Booked for your pets</h2></div><Stethoscope size={25} className="panel-heading-icon" /></div>{veterinaryAppointments.length ? <div className="veterinary-history-list">{veterinaryAppointments.map((appointment) => <div className="veterinary-history-row" key={appointment.id}><span className="veterinary-history-icon"><PawPrint size={17} weight="fill" /></span><div><strong>{appointment.petName}</strong><small>{veterinaryLabel(appointment.serviceType)} · {new Date(appointment.scheduledAt).toLocaleString(undefined, { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" })}</small></div><span className={`status-pill status-pill--${appointment.status === "pending" ? "pending" : "ready"}`}>{appointment.status}</span></div>)}</div> : <div className="empty-panel veterinary-empty"><Stethoscope size={25} /><p>No veterinary services booked yet.</p><small>New requests will appear here with the pet and scheduled care.</small></div>}</div></section>}

          {activeTab === "orders" && <section className="dashboard-tab-content"><div className="orders-panel dashboard-panel"><div className="panel-heading"><div><span className="eyebrow">Your purchases</span><h2>Order history</h2></div><ShoppingBag size={25} className="panel-heading-icon" /></div>{orders.length ? <div className="order-history-list">{orders.map((order) => <article className="order-history-card" key={order.id}><div className="order-history-top"><div><strong>Order #{String(order.id).padStart(5, "0")}</strong><small>{new Date(order.createdAt).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })}</small></div><span className={`order-status order-status--${order.status}`}>{order.status}</span><strong className="order-total">${(order.totalCents / 100).toFixed(2)}</strong></div><div className="order-history-items">{order.items.map((item) => <div className="order-history-item" key={item.id}><span>{item.name}</span><small>{item.quantity} × ${(item.priceCents / 100).toFixed(2)}</small></div>)}</div><div className="order-history-footer"><span><MapPin size={14} /> {order.city}</span><span><CheckCircle size={14} weight="fill" /> {order.status === "confirmed" ? "Order confirmed" : order.status}</span></div></article>)}</div> : <div className="empty-panel empty-panel--large"><ShoppingBag size={34} /><h3>No orders yet</h3><p>Your future purchases will appear here after checkout.</p><button className="button button--primary" onClick={() => navigate("/cart")}>Visit the store <CaretRight size={18} /></button></div>}</div></section>}
